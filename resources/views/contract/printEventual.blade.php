...
<div class="doc">
    ...
    <div class="content">
        <p class="text" style="text-indent: 0">
            Conste por el presente “Contrato Eventual de Prestación de Servicios” que suscriben por una parte la <span class="title-text">PASTELERIA VICTORIA'S</span>, representado legalmente por el <span class="title-text" style="text-align: justify;">Sr. {{ Util::fullName($mae->employee) }} con C.I. No. {{ Util::ciExt($mae->employee) }}, en su calidad de {{ $mae->position->name }} de la PASTELERIA VICTORIA'S</span>, designado mediante Resolución Suprema Nº {{ $company->directors_designation_number }} de fecha {{ Carbon::parse($company->directors_designation_date)->ISOFormat('LL') }}, y por otra parte <span class="title-text up">{{ Util::fullName($contract->employee) }} con C.I. N° {{ Util::ciExt($contract->employee) }} </span>, mayor de edad y hábil por derecho, con domicilio en la ciudad de {{ $contract->employee->location }}, que en lo sucesivo se denominará el <span class="title-text">“CONTRATADO”</span> quienes celebran el presente contrato eventual, de acuerdo a los términos y condiciones siguientes:
        </p>
        <p>
