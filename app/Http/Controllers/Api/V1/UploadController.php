...
class UploadController extends Controller
{
    ...
    public function file1(Request $request)
    {
        ...
        // dd($file);
        $nombre_unico = 'photo/'.uniqid().'.'.$file->getClientOriginalExtension();
        // dd($nombre_unico);
        Storage::disk('uploads')->put($nombre_unico,  File::get($file));
        return 'uploads/'.$nombre_unico;
    }
    public function file2(Request $request)
    {
        ...
        //dd($file);
        $nombre_unico = 'curriculum/'.uniqid().'.'.$file->getClientOriginalExtension();
        // dd($nombre_unico);
        Storage::disk('uploads')->put($nombre_unico,  File::get($file));
        return 'uploads/'.$nombre_unico;
    }
    public function file3(Request $request)
    {
        //dd($request->file);
        //$file = $request->file('file3');
        $file = $request->file('file');
        //dd($file);
        $nombre_unico = 'adjunto/'.uniqid().'.'.$file->getClientOriginalExtension();
        // dd($nombre_unico);
        Storage::disk('uploads')->put($nombre_unico,  File::get($file));
        return 'uploads/'.$nombre_unico;
    }
